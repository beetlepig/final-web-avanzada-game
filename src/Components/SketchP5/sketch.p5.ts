...
export const sketch = (width: number, height: number, props: SketchProps) => {
    return function (p: customP5Functions) {
        ...
        async function fixedUpdate() {
            while (mounted) {
                await checkExplosionCollision();
                await checkBulletsCollision();
            }
        }

        function checkBulletsCollision(): Promise<void> {
        ...
        function checkExplosionCollision(): Promise<void> {
            return new Promise<void>(resolve => {
                if (enemySpaceShip) {
...
class EnemyShip {
    ...
    async releaseExplosion(): Promise<void> {
            for (let i = 0; i < this.p5Instance.random(2, 6); i++) {
                if (this.alive) {
                    this.explosionWaves.push(new ExplosionWave(this.pos.copy(), this.diameter, this.p5Instance.random(1, 1), this.p5Instance));
                    if (i < 5) {
                        await this.delayBetweenExplosions(this.p5Instance.random(500, 2000));
                    } else {
                        await this.delayBetweenExplosions(100);
                    }
                }
            }
    }
...
class ExplosionWave {
    ...
    async updateExpansion() {
        while (this.alive) {
            await this.explosionExpansion();
        }
    }

    explosionExpansion(): Promise<void> {
        return new Promise<void>(resolve => {
